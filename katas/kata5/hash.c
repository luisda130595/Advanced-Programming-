...
static void hashDoInsert(HashInt *hash, char *key)
{
    ...
    currentHashElement->elements[currentHashElement->count].value = 0; 
    currentHashElement->count += 1;

}

static void hashDoUpdate(HashInt *hash, char *key)
{
...
void hashInsert(HashInt *hash, char *key)
{
    int * oldValue = hashGet(hash,key);
    if(oldValue == NULL){
        hashDoInsert(hash,key);
    }
    else{
        hashDoUpdate(hash,key);
    }
}
